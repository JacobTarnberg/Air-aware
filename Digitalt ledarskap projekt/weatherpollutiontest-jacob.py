...
def fetch_city_weather(lat: float, lon: float):
    """Fetch live meteorological forecast parameters from SMHI."""
    try:
        url = (
...
with header_col1:
    ...
    st.caption("Synchronized live air quality, live SMHI weather, and historical pollen benchmarks by Region & City[cite: 1, 3].")
...
with pollen_ctrl_col:
    # Let runner pick today (default) or any target running date
    target_run_date = st.date_input("Run Date for Comparison", value=date.today())
    
    # Calculate exact date 1 year ago (handling leap years safely)
    try:
...
# Search for the pollen observation for that day
...
if not df_pollen.empty and "Date_Clean" in df_pollen.columns:
    ...
    if not exact_match.empty:
        ...
        # 2. Find nearest observation within the previous year
        ...
        # Only accept if within 14 days of the seasonal period
...
with pollen_metric_col:
    if not pollen_day_df.empty:
        ...
        # Severity categorization for runners
        if max_level == 0:
        ...
        # Plotly chart of pollen species levels
