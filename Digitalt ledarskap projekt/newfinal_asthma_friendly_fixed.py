    ...
    try:
        ...
        for (city, coords), item in zip(city_items, payload):
            ...
            rows.append({
                ...
                "Air Quality Index": aqi, "Level": meta["level"],
...
def live_panel():
    ...
    st.subheader("What is in the air?")
    ...
    for col, name in zip(cols, card_names):
        ...
        col.markdown(
            ...
            <div class="metric-card" style="border-top:5px solid {pmeta['color']};">
                ...
                <div class="small-muted">{info['short']}</div>
                <div class="metric-value">{value_text}</div>
                <div class="metric-status" style="color:{pmeta['color']};">{status_text}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with st.expander("What do these pollutants mean?", expanded=False):
        for name, info in POLLUTANTS.items():
            st.markdown(
                f"**{name} — {info['short']}**  \n"
                f"**In simple words:** {info['plain']}  \n"
                f"**Asthma note:** {info['asthma']}  \n"
                f"**Common sources:** {info['sources']}"
            )
    ...
    for name, info in POLLUTANTS.items():
        ...
        if isinstance(score, (int, float)):
            ...
            chart_rows.append({"Pollutant": name, "Air Quality Index": score, "Level": meta["level"]})
    ...
    if chart_rows:
        ...
        fig = px.bar(
            ...
            title="Which pollutant is driving the Air Quality Index?",
        )
...
with st.expander("ℹ️ How the levels work", expanded=False):
    st.markdown(
        """
        This dashboard uses the **European Air Quality Index** supplied by Open-Meteo/CAMS and presents it as five easy-to-read public-facing levels. The same names and colors are used consistently in the cards, graph and map:
        ...
        The official European index has six bands (Good, Fair, Moderate, Poor, Very Poor and Extremely Poor).
        Here, the labels are intentionally simplified for general audiences.

        **Timing:** Open-Meteo current air-quality conditions are based on 15-minute model data. The CAMS European air-quality forecast has hourly temporal resolution. Therefore, the interface can refresh every second, but the source values should not be expected to change every second.
        """
    )
