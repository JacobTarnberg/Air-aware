...
st.write(
    "Välj en stad för att se den senaste "
    "tillgängliga väderprognosen från SMHI."
)
...
city = st.selectbox(
    ...
    index=None,
    placeholder="Börja skriva stadens namn..."
)
...
if city is None:
    ...
    st.info(
        "Skriv början av en stads namn i fältet ovan "
        "och välj sedan en stad."
    )
    ...
    try:
        ...
        st.caption(
            "Källa: SMHI – SNOW1g meteorologisk prognos"
        )

    except requests.RequestException as error:

        st.error(
            f"Kunde inte hämta väderdata från SMHI: {error}"
        )
        ...
        st.error(
            f"Ett oväntat fel uppstod i SMHI:s data: {error}"
        )